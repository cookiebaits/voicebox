...
const CAPTURES: Capture[] = [
  ...
  },
  {
    ...
    seed: 37,
    transcriptRaw:
      "tech overview for the readme six TTS engines qwen3 kokoro chatterbox luxtts customvoice and chatterbox turbo whisper for STT in five sizes from base up to large and a turbo variant one local LLM qwen 3.5 shared runtime across all of them one model directory one GPU story no fragmented caches pick the right model per job speed on CPU laptops quality on an M series mac all switchable per generation",
    transcriptRefined:
      "Tech overview for the README: six TTS engines — Qwen3, Kokoro, Chatterbox, LuxTTS, CustomVoice, and Chatterbox Turbo. Whisper for STT, in five sizes from Base up to Large, plus a Turbo variant. One local LLM, Qwen 3.5, with a shared runtime across all of them. One model directory, one GPU story, no fragmented caches. Pick the right model per job — speed on CPU laptops, quality on an M-series Mac, switchable per-generation.",
    durationMs: 34000,
    ago: '1 hr ago',
